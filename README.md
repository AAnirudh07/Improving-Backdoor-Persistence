...
5. **Benign Overwrite after Durable Backdoor:** Apply the same benign training protocol to the durable backdoor model, and evaluate retention of the improved trigger-backdoor.
...
- The CE loss notation suggests that prompt tokens should be masked (e.g. log(fθ(yb,i|xb,i)))   
    ...
    - Furthermore, the paper also mentions the prompt tokens (not response) are used: "EL(xb,j) and EL(xc,i) are the token embeddings of the backdoored prompts and clean prompts produced by the final transformer layer of the LLM fθ."
